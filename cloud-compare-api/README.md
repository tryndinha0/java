...
| Decisão | Motivo |
|---|---|
| Java 17 + Spring Boot 3 | Garante suporte de longo prazo (LTS), melhor desempenho da JVM e acesso a recursos modernos de linguagem. |
...
| Spring Boot Actuator (`/actuator/health`) | Endpoint de health check padrão, compatível com o health check de load balancer/monitoramento dos três provedores. |
...
Pacote base `br.tcc`, separado por camada.
...
Use a mesma classe de instância (vCPU/RAM) e o mesmo tamanho de disco nos três.
...
endpoint `/actuator/health` e para os logs do container.
...
adicione os cenários que faltam (POST/PUT/DELETE, diferentes níveis de
